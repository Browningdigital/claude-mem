/**
 * OCI ARM Instance Provisioner — Cloudflare Worker
 *
 * Runs on a 1-minute cron trigger. Checks if a free-tier ARM instance
 * exists in OCI us-chicago-1, and if not, tries to launch one.
 * Cycles through all 3 availability domains using stateless rotation.
 *
 * This replaces the bash daemon which dies when the container sleeps.
 * Cloudflare cron triggers run 24/7 regardless of any other infra.
 *
 * Deploy:
 *   cd cloud-node/worker
 *   wrangler deploy -c wrangler-oci.toml
 *
 * Secrets (set before deploying):
 *   wrangler secret put OCI_PRIVATE_KEY -c wrangler-oci.toml
 *   wrangler secret put OCI_SSH_PUBLIC_KEY -c wrangler-oci.toml
 *   wrangler secret put SUPABASE_URL -c wrangler-oci.toml
 *   wrangler secret put SUPABASE_KEY -c wrangler-oci.toml
 */
...
export default {
  async scheduled(event: ScheduledEvent, env: Env, ctx: ExecutionContext) {
    ...
    // Step 1: Check if instance already exists (any active state)
    try {
      ...
      // Continue to try launching anyway — don't let a list error block us
    }

    // Step 2: Pick AD — stateless rotation (minute mod AD count)
    const adIndex = new Date().getMinutes() % ads.length;
    ...
    try {
      ...
      if (result && result.id) {
        ...
        await log(env, "success",
          `INSTANCE CREATED! id=${result.id} ad=${ad} ip=${ip} state=${result.lifecycleState}`
        );
        return;
      }
      ...
      if (/out of (?:host )?capacity/i.test(msg) || /InternalError/i.test(msg)) {
        await log(env, "retry", `Out of capacity — ${ad}`);
      } else if (/TooManyRequests|429/i.test(msg)) {
        await log(env, "rate_limit", `Rate limited — ${ad}`);
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/status") {
      // Quick status check — does an instance exist?
      try {
        const region = env.OCI_REGION || "us-chicago-1";
        const host = `iaas.${region}.oraclecloud.com`;
        const instances = await ociGet(
        ...
        return Response.json({ status: "searching", message: "No instance yet — cron retrying every minute" });
      } catch (e: any) {
...
// ═══════════════════════════════════════════════════
// OCI REST API — Request Signing (RSA-SHA256)
// ═══════════════════════════════════════════════════
...
let _cachedPem: string = "";

async function importPKCS8(pem: string): Promise<CryptoKey> {
  // Cache the imported key — importing is expensive
