...
// Capture formatTool reference at import time, before mock.module() from other
// test files can replace it on the logger object (Bun test isolation issue).
// Using .bind() captures the actual function, not a property lookup.
const formatTool = logger.formatTool.bind(logger);
